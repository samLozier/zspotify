...
from database.zspotify_db import Podcast, Episode, engine
...
ROOT_PATH = "/Users/sl/Desktop/zspotify/"
ROOT_PODCAST_PATH = "podcast-data/"
...
def get_show_info(show_url):
    show_id = regex_input_for_urls(show_url)[4]
    token = SESSION.tokens().get("user-read-email")
    response = json.loads(
        requests.get(
...
def load_show_list():
    with open(Path(__file__).parents[0] / "podcast_data" / "new_podasts.csv", "r") as csv_file:
        reader = csv.reader(csv_file)
        shows = [r[0] for r in reader]
    return shows


def show_obj(show_info_dict):
    pc = Podcast()
    print(show_info_dict)
...
def download_episode(episode_id_str):
    ...
    podcast_name = slugify(podcast_name)
    ...
    if podcast_name is None:
        ...
        print(ROOT_PODCAST_PATH + extra_paths + filename)
        file_path = ROOT_PODCAST_PATH + extra_paths + filename + ".wav"
        ...
        converted_file_path = convert_audio_format(
            ROOT_PODCAST_PATH + extra_paths + filename + ".wav"
        )
...
def search(search_term):
    """Searches Spotify's API for relevant podcast_data"""
    token = SESSION.tokens().get("user-read-email")
...
def convert_audio_format(filename):
    ...
    if QUALITY == AudioQuality.VERY_HIGH:
        ...
        bitrate = "160k"
    converted_file_path = Path(filename).with_suffix("." + MUSIC_FORMAT)
    raw_audio.export(converted_file_path, format=MUSIC_FORMAT, bitrate=bitrate)
    return converted_file_path
...
def download_track(
    ...
    try:
        ...
        if prefix:
            ...
            song_name = (
                f"{_artist} - {album_name} - {_track_number}. {name}.{MUSIC_FORMAT}"
            )
...
login()
shows = load_show_list()
podcast_info = [get_show_info(show) for show in shows]
for show in podcast_info:
    clean_show = {
        k: v for k, v in show.items() if k in Podcast.__table__.columns.keys()
    }
    clean_show['slug_name'] = slugify(clean_show['name'])
    if (
        existing_show := session_orm.query(Podcast)
        .filter(Podcast.id == clean_show["id"])
        .first()
    ) is not None:
        session_orm.query(Podcast).filter(Podcast.id == clean_show["id"]).update(
            clean_show
        )
        session_orm.commit()
    else:
        session_orm.add(Podcast(**clean_show))
        session_orm.commit()
# session_orm.commit()
